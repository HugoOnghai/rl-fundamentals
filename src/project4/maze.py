...
class Maze():
    ...
    def plot(self,state,action,extra_lines=3):
        cell = self.idx2cell[int(state/8)] 
        desc = self.map.tolist()

        desc[cell[0]] = desc[cell[0]][:cell[1]] + '\x1b[1;32m' + desc[cell[0]][cell[1]] + '\x1b[0m' + desc[cell[0]][cell[1]+1:]

        # hugo edit to make animation cleaner
        # move cursor up to overite previous frame instead of having a long list of them
        # 2 extra lines for the "action: " line and spacing
        sys.stdout.write(f"\033[{len(desc) + extra_lines}A\r")
        sys.stdout.flush()

        # hugo edit clear each line before writing
        for _ in range(len(desc) + extra_lines):
            sys.stdout.write("\033[2K\n") # erase line, then move down
        sys.stdout.write(f"\033[{len(desc) + extra_lines}A\r") # go back up again
...
if __name__ == '__main__':
    ...
    state = maze.reset()
    print("\n" * (maze.dim[0]+3), end="") # hugo change, give extra lines at start to support cursor move up
